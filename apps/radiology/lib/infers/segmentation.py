...
class Segmentation(BasicInferTask):
    ...
    def pre_transforms(self, data=None) -> Sequence[Callable]:
        if self.dimension==2:
            t = [
                ...
                EnsureChannelFirstd(keys=("image"), channel_dim=-1),  # images have shape (h,w,1)
    ...
    def post_transforms(self, data=None) -> Sequence[Callable]:
        t = [
            ...
            AsDiscreted(keys="pred", argmax=True),  # binarize
        ]
        ...
        if self.dimension == 2:
            t.append(AsChannelLastd(keys="pred")) # move channel dim from first to last: (1,h,w) -> (h,w,1)
        else:
            t.append(Restored(keys="pred", ref_image="image"))  # restore orientation
