...
class RestoreLabelDimd(MapTransform):
    def __init__(self, keys: KeysCollection, allow_missing_keys: bool = False):
        """
        Change the shape of the predicted images from (1, x, y) to (x, y, 1) as for the labels saved by 3D Slicer.
    ...
    def __call__(self, data):
        d: Dict = dict(data)
        for key in self.key_iterator(d):
            d[key] = torch.moveaxis(d[key], 0, -1)
        return d
